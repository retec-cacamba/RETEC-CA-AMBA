(() => {
  ...
  if (grid) {
    ...
    document.querySelectorAll('.size-checkout').forEach(a => {
      ...
      a.href = waUrl(`Olá! Vim pelo site da Disk Caçamba Curitiba e gostaria de reservar uma caçamba de ${size}. Também quero saber sobre entrega em até 2 horas e a opção de mão de obra por R$ 80,00. Meu bairro/CEP é: `);
      bindWhatsAppLink(a, 'whatsapp_size_click', { size });
    });
  }

  const subprefs = ['Centro','Água Verde','Batel','Bigorrilho','Boqueirão','Cajuru','Capão Raso','CIC','Hauer','Juvevê','Mercês','Portão','Pinheirinho','Santa Felicidade','Uberaba','Xaxim','São José dos Pinhais','Colombo','Pinhais','Araucária','Campo Largo','Fazenda Rio Grande','Almirante Tamandaré','Campo Magro','Quatro Barras'];
  ...
  if (leadForm) {
    leadForm.addEventListener('submit', e => {
      ...
      const msg = `Olá! Vim pelo site da Disk Caçamba Curitiba.
Nome: ${fd.get('name')}
  ...
  document.querySelectorAll('.map-point-v22').forEach(point => {
    ...
    point.addEventListener('click', () => {
      ...
      reportWhatsAppConversion(
        waUrl(`Olá! Vim pelo mapa do site da Disk Caçamba Curitiba e gostaria de consultar entrega para ${point.dataset.region}. Meu bairro/CEP é: `)
      );
    });
  });
